...
<section>
<form class="box" action="../Controller/addemp.php"onsubmit="return validation()" method="post">

      <div id="error_message"></div>
      <h1>Employee Registration</h1>
      <input type="text"id="uname" name="ouname" placeholder="Employee Id" />
	  ...
	  <input type="text"id="spe" name="dspecial" placeholder="Gender" />
	   <input type="date" id="rono"name="roono" placeholder="DOB" />
      <input type="submit" name="empsignup" value="Sign-Up" />
    </form>
		...
		function validation(){
      	...
      	var uid = document.getElementById('uname').value;
			...
			if((user.length <= 2) || (user.length > 25)) {
				document.getElementById('error_message').innerHTML =" ** name lenght must be between 3 and 25";
				return false;
			}
			if(!isNaN(user)){
...
if((speciality.length <=3) || (speciality.length  >7)) {
      ...
      if(Dob == ""){
        document.getElementById('error_message').innerHTML =" ** Please fill the DOB field";
        return false;
      }
